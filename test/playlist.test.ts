...
import M3U8Parse, { MainPlaylist, MediaPlaylist, MediaSegment, AttrList } from '../lib/index.js';
...
describe('M3U8Playlist', () => {
    ...
    before(async () => {
        ...
        testIndex = M3U8Parse(await readFixture('enc.m3u8'), { type: 'media' });
        testIndexAlt = M3U8Parse(await readFixture('enc-discont.m3u8'), { type: 'media' });
        testIndexSingle = M3U8Parse(await readFixture('enc-single.m3u8'), { type: 'media' });
        testIndexLl = M3U8Parse(await readFixture('ll.m3u8'), { type: 'media' });
        mainIndex = M3U8Parse(await readFixture('variant_v4.m3u8'), { type: 'main' });
    });
    ...
    describe('constructor', () => {
        ...
        it('performs object to Map conversion', async () => {
            ...
            const index = M3U8Parse(await readFixture('variant_v4.m3u8'));
    ...
    describe('#toString()', () => {
        ...
        it('should output valid main playlist files', async () => {
            ...
            const mainIndexV6 = M3U8Parse(await readFixture('variant_v6.m3u8'));
